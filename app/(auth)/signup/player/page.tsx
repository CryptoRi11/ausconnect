...
export default function PlayerSignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data, error: signUpError } = await signUpAndAutoLogin(email, password, "player");
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-lg">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-black text-white mb-2">Create your player profile</h1>
            <p className="text-white/50 text-sm">Get in front of US college coaches</p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" placeholder="Jake" className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-gold/50 transition-all" />
                </div>
                <div>
                  <label className="block text-white/70 text-sm font-medium mb-2">Last Name</label>
                  <input type="text" placeholder="Morrison" className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-gold/50 transition-all" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:border-gold/50 transition-all appearance-none">
                    ...
                    {positions.map(p => <option key={p} value={p} className="bg-navy">{p}</option>)}
                  </select>
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" placeholder="2026" min="2024" max="2030" className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-gold/50 transition-all" />
                </div>
                <div>
                  <label className="block text-white/70 text-sm font-medium mb-2">Height</label>
                  <input type="text" placeholder={`6'3"`} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-gold/50 transition-all" />
                </div>
              </div>
